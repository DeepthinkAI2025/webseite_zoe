...
export const metadata: Metadata = buildMetadata({
  title: 'Photovoltaik Installation Mainz – Planung & Montage',
  ...
  canonicalPath: '/standorte/mainz'
});

export default function MainzStandortPage(){
  const usp = [
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      <JsonLd id="ld-breadcrumb-standort-mainz" data={breadcrumbJsonLd([{ name: 'Start', url: '/' }, { name: 'Standorte', url: '/standorte' }, { name: 'Mainz', url: '/standorte/mainz' }])} />
      <JsonLd id="ld-servicearea-mainz" data={cityServiceJsonLd({
        name: localBiz.name,
        url: 'https://www.zoe-solar.de/standorte/mainz',
        city: 'Mainz',
        country: localBiz.country,
        areaServed: ['Mainz','Rheinland-Pfalz','Rhein-Main'],
        lat: 49.992863,
        lon: 8.247253,
        serviceName: 'Photovoltaik Installation Mainz',
        serviceDescription: 'Regionale Planung & Installation von PV & Speicher Systemen in Mainz.'
      })} />
      <JsonLd id="ld-faq-mainz" data={{ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: faqs.map(f => ({ '@type': 'Question', name: f.q, acceptedAnswer: { '@type': 'Answer', text: f.a } })) }} />
      <header className="text-center mb-10">
  <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Photovoltaik Installation Mainz</h1>
  <p className="mt-4 text-neutral-600 max-w-2xl mx-auto">Planung → Installation → Monitoring – wirtschaftlich optimiert für regionale Bedingungen.</p>
      ...
      <section className="mt-12"><h2 className="text-xl font-semibold mb-4">Regionale Kennzahlen & Kontext</h2><div className="grid md:grid-cols-2 gap-6 text-sm text-neutral-700"><ul className="space-y-2"><li><strong>Globalstrahlung:</strong> 1185 kWh/m²</li><li><strong>Spezifischer Ertrag:</strong> 950–1085 kWh/kWp</li><li><strong>Speicher Hinweis:</strong> Weinlagen Reflexion leicht positiv → Speicher 7–9 kWh</li></ul><ul className="space-y-2"><li><strong>Netzanschluss Ablauf:</strong> 4–6 Wochen</li><li><strong>Verschattungsrisiko:</strong> low</li><li><strong>Dachmix:</strong> Steildächer mit geringer Verschattung</li></ul></div></section>
    </main>
  );
}
