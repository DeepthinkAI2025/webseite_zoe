...
export const metadata: Metadata = buildMetadata({
  title: 'Photovoltaik Installation Lübeck – Planung & Montage',
  ...
  canonicalPath: '/standorte/luebeck'
});

export default function LbeckStandortPage(){
  const usp = [
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      <JsonLd id="ld-breadcrumb-standort-luebeck" data={breadcrumbJsonLd([{ name: 'Start', url: '/' }, { name: 'Standorte', url: '/standorte' }, { name: 'Lübeck', url: '/standorte/luebeck' }])} />
      <JsonLd id="ld-servicearea-luebeck" data={cityServiceJsonLd({
        name: localBiz.name,
        url: 'https://www.zoe-solar.de/standorte/luebeck',
        city: 'Lübeck',
        country: localBiz.country,
        areaServed: ['Lübeck','Schleswig-Holstein'],
        lat: 53.865467,
        lon: 10.686559,
        serviceName: 'Photovoltaik Installation Lübeck',
        serviceDescription: 'Planung & Installation von Photovoltaik & Speicher Lösungen in Lübeck.'
      })} />
      <JsonLd id="ld-faq-luebeck" data={{ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: faqs.map(f => ({ '@type': 'Question', name: f.q, acceptedAnswer: { '@type': 'Answer', text: f.a } })) }} />
      <header className="text-center mb-10">
  <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Photovoltaik Installation Lübeck</h1>
  <p className="mt-4 text-neutral-600 max-w-2xl mx-auto">Planung → Installation → Monitoring – wirtschaftlich optimiert für regionale Bedingungen.</p>
      ...
      <section className="mt-12"><h2 className="text-xl font-semibold mb-4">Weitere Standorte (Region)</h2><ul className="list-disc pl-5 space-y-1 text-sm"><li><a className="text-blue-600 underline" href="/standorte/hamburg">Photovoltaik Hamburg</a></li><li><a className="text-blue-600 underline" href="/standorte/kiel">Photovoltaik Kiel</a></li><li><a className="text-blue-600 underline" href="/standorte/rostock">Photovoltaik Rostock</a></li></ul></section>
    
      <section className="mt-12"><h2 className="text-xl font-semibold mb-4">Regionale Kennzahlen & Kontext</h2><div className="grid md:grid-cols-2 gap-6 text-sm text-neutral-700"><ul className="space-y-2"><li><strong>Globalstrahlung:</strong> 1055 kWh/m²</li><li><strong>Spezifischer Ertrag:</strong> 880–990 kWh/kWp</li><li><strong>Speicher Hinweis:</strong> Wechselhafte Bewölkung → 6–9 kWh Speicher</li></ul><ul className="space-y-2"><li><strong>Netzanschluss Ablauf:</strong> 6–9 Wochen</li><li><strong>Verschattungsrisiko:</strong> medium</li><li><strong>Dachmix:</strong> Altstadtnähe teils Gauben / Verschattung</li></ul></div></section>
    </main>
  );
}
