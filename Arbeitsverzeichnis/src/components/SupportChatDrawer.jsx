...
export default function SupportChatDrawer({ isOpen: propIsOpen, onToggle, onClose, className = '' }) {
  ...
  // Sync with prop
  useEffect(() => {
    if (propIsOpen !== undefined) setOpen(propIsOpen);
  }, [propIsOpen]);
  ...
  useEffect(() => {
    const onOpen = () => handleToggle();
    const onCloseEvent = () => handleToggle();
    window.addEventListener('open-support-chat', onOpen);
    window.addEventListener('close-support-chat', onCloseEvent);
    return () => {
      window.removeEventListener('open-support-chat', onOpen);
      window.removeEventListener('close-support-chat', onCloseEvent);
    };
  }, []);
      ...
      <FocusLock>
        <div ref={panelRef} className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl border-l border-neutral-200 flex flex-col outline-none focus-visible:focus-ring" tabIndex="-1" style={{ transition: prefersReducedMotion ? 'none' : 'transform 300ms ease-out' }}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200">
            ...
            <Button variant="plain" aria-label="Schließen" className="p-2 rounded hover:bg-neutral-100 focus-visible:focus-ring" onClick={handleToggle}>
              <X className="w-5 h-5" />
            </Button>
          </div>
