...
const ServiceDrawer = ({ isOpen: propIsOpen, onToggle, onClose, className = '' }) => {
    const [isOpen, setIsOpen] = useState(() => {
        if (propIsOpen !== undefined) return propIsOpen;
    ...
    useEffect(() => {
        const onScroll = () => {
            if (window.scrollY > 60 && isOpen) handleToggle();
        };
    ...
    useEffect(() => {
        const onResize = () => {
            if (window.innerWidth < 1024 && isOpen) handleToggle();
        };
